...
<script setup>
	...
	const emit = defineEmits([ 'optionSelected', 'update:modelValue', 'tag', 'objectReturned' ]);
	...
	onMounted(async () => {
		...
		if(!props.multiple) selectedElements.value = props.modelValue;
			...
			if(props.modelValue) {
				...
				// if props.modelValue is an array
				if(Array.isArray(props.modelValue)) {
					selectedElements.value = [ ...selectedElements.value, ...props.modelValue ];
				} else {
					// if props.modelValue is a string or number
	...
	const asyncFind = async (query) => {
		...
		if(error.value) {
			...
			elements.value = [ ...new Map(found.map((item) => [ item.id, item ])).values() ];
		}
		isLoading.value = false;
	...
	const updateElement = (element) => {
		...
		if(typeof selectedElements === 'object') {
			emit('update:modelValue', selectedElements.value.id);
			emit('objectReturned', selectedElements.value);
		} else {
